...
impl super::ObjC {
    ...
    pub fn borrow_mut<T: AnyHostObject + 'static>(&mut self, object: id) -> &mut T {
        ...
        let mut host_object: &mut Aho = &mut *self.objects.get_mut(&object).unwrap_or_else(|| panic!("borrow_mut called on deallocated object {:?} (type: {})", object, std::any::type_name::<T>())).host_object;
        loop {
